...
data/power_prices.csv       electricity rates by county (generated)
...
## The electricity cost layer

`data/power_prices.csv` is generated by `scripts/build_power_prices.py` from
U.S. EIA [Form 861](https://www.eia.gov/electricity/data/eia861/), which is
public domain. It covers **247 of 254 counties** and shows the change in
residential rates from 2019 to 2024.

### What the number is, and is not

**Texas has no county electricity price.** Price is not set by county: in the
deregulated ERCOT area a household picks among many retail providers and
plans, and elsewhere a co-op or municipal utility sets its own rate. EIA
reports revenue and sales per utility per *state*, never per county.

So for each county the script computes:

```
price = sum(residential revenue) / sum(residential sales)
```

over the utilities that operate in that county. The result is a
**sales-weighted average of the rates charged by utilities operating there**.
It is a fair proxy for what those utilities charge. It is **not** the average
bill paid in that county, and two counties served by the same utilities get
the same number even where real bills differ. The popup says so on every
county.

Comparing two years on the same basis is more robust than either year alone,
because a bias in how utilities are attributed to counties largely cancels in
the ratio.

### This layer is not about data centers

Residential rates rose across Texas over this period for many reasons —
fuel costs, transmission buildout, and the aftermath of Winter Storm Uri among
them. **The map deliberately does not claim data centers caused it.** The
layers are there to be compared, not conflated, and the legend says as much.
...
The script downloads the EIA archives, handles the fact that column layouts
differ between years, and rewrites the CSV.
